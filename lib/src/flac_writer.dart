...
class FlacWriter extends MetadataWriter {
  ...
  _FlacData? _parseFlacFile(Uint8List bytes) {
    ...
    while (offset < bytes.length) {
      ...
      blocks.add(
        _MetadataBlock(type: blockType, isLast: isLast, data: blockData),
      );
  ...
  Uint8List _replaceVorbisComment(_FlacData flacData, Uint8List vorbisComment) {
    ...
    for (int i = 0; i < flacData.blocks.length; i++) {
      ...
      if (block.type == 4) {
        ...
        newBlocks.add(
          _MetadataBlock(type: 4, isLast: block.isLast, data: vorbisComment),
        );
    ...
    if (!hasVorbisComment) {
      ...
      newBlocks.add(_MetadataBlock(type: 4, isLast: true, data: vorbisComment));
    }
